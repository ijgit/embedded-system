...
void DoorLock::setPassword(int *password)
{ // 비밀번호 설정
  for (int i = 0; i < 4; i++)
  {
...
void DoorLock::detectFlag()
{ // 플래그 감지(플래그는 버튼이 눌렸는지 안눌렸는지 상태)
  toggle[4].play();
  state = toggle[4].getFlag();
}

int DoorLock::getResult()
{ // 결과값 받아오기
  return result;
}
void DoorLock::setResult(int n)
{ // 결과값 설정
  result = n;
}
